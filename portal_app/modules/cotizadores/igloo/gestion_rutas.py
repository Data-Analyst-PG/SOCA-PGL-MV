...
Homologado con captura_rutas.py:
  - Orden dinámico de secciones según obtener_config_tipo_ruta()
      ...
      VACIO / DOM MEX → solo Ruta MX (sin Cruce)
  - _preview_edicion() centraliza banner + KPIs + desglose vía mostrar_resultados_igloo()
  - Historial de modificaciones guarda el SNAPSHOT COMPLETO de la versión anterior
    ...
    exactamente por qué cambió cualquier costo o ingreso.
...
# ─────────────────────────────────────────────
def _seccion_info_general_edicion(idx_sel: str, ruta: dict) -> dict:
    ...
    return {"fecha": fecha, "tipo": tipo, "cliente": cliente, "modo_viaje": modo_viaje}


def _seccion_cruce_edicion(idx_sel: str, ruta: dict) -> dict:
    st.markdown("### 🛂 Cruce")
    c1, c2, c3, c4 = st.columns(4)
    moneda_cruce       = c1.selectbox(
        "Moneda Ingreso Cruce", ["MXP", "USD"],
        index=["MXP", "USD"].index(str(ruta.get("Moneda_Cruce", "MXP"))) if str(ruta.get("Moneda_Cruce", "MXP")) in ["MXP", "USD"] else 0,
        key=f"ig_ed_mon_cruce_{idx_sel}",
    )
    ingreso_cruce      = c2.number_input(
        "Ingreso Cruce", min_value=0.0,
        value=float(safe_number(ruta.get("Cruce_Original", 0))),
        key=f"ig_ed_ing_cruce_{idx_sel}",
    )
    moneda_costo_cruce = c3.selectbox(
        "Moneda Costo Cruce", ["MXP", "USD"],
        index=["MXP", "USD"].index(str(ruta.get("Moneda Costo Cruce", "MXP"))) if str(ruta.get("Moneda Costo Cruce", "MXP")) in ["MXP", "USD"] else 0,
        key=f"ig_ed_mon_cc_{idx_sel}",
    )
    costo_cruce        = c4.number_input(
        "Costo Cruce", min_value=0.0,
...
def _seccion_ruta_mx_edicion(idx_sel: str, ruta: dict, tipo: str) -> dict:
    ...
    c1, c2, c3, c4 = st.columns(4)
    moneda_ingreso = c1.selectbox(
        "Moneda Ingreso Flete", ["MXP", "USD"],
        index=["MXP", "USD"].index(str(ruta.get("Moneda", "MXP"))) if str(ruta.get("Moneda", "MXP")) in ["MXP", "USD"] else 0,
        key=f"ig_ed_mon_ing_{idx_sel}",
    )
    ingreso_flete  = c2.number_input(
        "Ingreso Flete", min_value=0.0,
        value=float(safe_number(ruta.get("Ingreso_Original", 0))),
        key=f"ig_ed_ing_flete_{idx_sel}",
    )
    km             = c3.number_input(
        "📏 Kilómetros", min_value=0.0,
        value=float(safe_number(ruta.get("KM", 0))),
        key=f"ig_ed_km_{idx_sel}",
    )
    casetas        = c4.number_input(
        "🛣️ Casetas (MXP)", min_value=0.0,
...
def _seccion_costos_fijos_edicion(idx_sel: str, ruta: dict) -> dict:
    ...
    puntualidad     = c4.number_input(
        "⏰ Puntualidad (MXP)", min_value=0.0,
...
def _seccion_otros_costos_edicion(idx_sel: str, ruta: dict) -> dict:
...
# GUARDAR CAMBIOS
# ─────────────────────────────────────────────
def _guardar_edicion(supabase, ruta: dict, nombre_usuario: str, valores: dict) -> None:
    ...
    # ── Historial: snapshot COMPLETO de la versión anterior ────────
    # Se guarda TODO el diccionario previo (menos "historial" para no anidar
    # infinitamente) para poder auditar exactamente qué cambió y por qué.
    historial_anterior = ruta.get("historial") or []
    if not isinstance(historial_anterior, list):
        historial_anterior = []

    snapshot_anterior = {k: v for k, v in ruta.items() if k != "historial"}

    nueva_entrada = {
        "timestamp": now_iso(),
        "usuario":   nombre_usuario,
        "motivo":    d["motivo"],
        "cambios_anteriores": snapshot_anterior,
    }
...
def render():
    ...
    with tab_edit:
        ...
        # ── Historial (solo lectura) — ahora con snapshot completo ─────
        ...
        if isinstance(historial, list) and historial:
            with st.expander(f"📜 Historial de modificaciones ({len(historial)})", expanded=False):
                for entrada in reversed(historial):
                    ...
                    if cambios:
                        st.caption(
                            f"📍 {cambios.get('Tipo','—')} · {cambios.get('Cliente','—')} · "
                            f"{cambios.get('Origen','')} → {cambios.get('Destino','')}"
                        )
                        c1, c2, c3 = st.columns(3)
                        c1.caption(f"Ingreso Total: **${safe_number(cambios.get('Ingreso Total', 0)):,.2f}**")
                        c1.caption(f"Costo Total: **${safe_number(cambios.get('Costo_Total_Ruta', 0)):,.2f}**")
                        c2.caption(f"Diesel: **${safe_number(cambios.get('Costo_Diesel_Camion', 0)) + safe_number(cambios.get('Costo_Diesel_Termo', 0)):,.2f}**")
                        c2.caption(f"Sueldo: **${safe_number(cambios.get('Sueldo_Operador', 0)):,.2f}**")
                        c3.caption(f"Costos Indirectos: **${safe_number(cambios.get('Costos_Indirectos', 0)):,.2f}**")
                        c3.caption(f"Ut. Neta: **${safe_number(cambios.get('Utilidad_Neta', 0)):,.2f}**")
                        with st.expander("Ver todos los campos de esta versión", expanded=False):
                            st.json(cambios)
                    st.divider()
        ...
        info = _seccion_info_general_edicion(idx_sel, ruta)
        tipo = info["tipo"]
        ...
        for seccion in orden:
            divider()
            if seccion == "cruce":
                datos_cruce = _seccion_cruce_edicion(idx_sel, ruta)
            elif seccion == "ruta_mx":
                datos_mx = _seccion_ruta_mx_edicion(idx_sel, ruta, tipo)

        divider()
        datos_costos_fijos = _seccion_costos_fijos_edicion(idx_sel, ruta)

        divider()
        datos_otros = _seccion_otros_costos_edicion(idx_sel, ruta)
        ...
        if st.session_state.get("igloo_revisar_edicion", False):
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("💾 Guardar Cambios", type="primary", use_container_width=True, key="igloo_save_edicion"):
                    _guardar_edicion(supabase, ruta, nombre_usuario, valores)
            with c2:
                if st.button("🗑️ Cancelar Edición", use_container_width=True, key="igloo_cancelar_edicion"):
                    st.session_state.igloo_revisar_edicion = False
                    st.session_state.pop("igloo_datos_edicion", None)
                    st.session_state.pop("igloo_calc_edicion", None)
                    st.rerun()
