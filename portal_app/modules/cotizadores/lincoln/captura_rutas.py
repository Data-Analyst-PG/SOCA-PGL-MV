...
- Empty: solo millas vacías (diesel + pago operador vacío, sin bono, sin ISR/IMSS)
- NB/SB/D2D: cálculo completo
Patrón visual alineado con Set Logis Plus.
"""
...
def _mostrar_resultados(r: dict, fd: dict) -> None:
    ...
    if es_empty:
        filas_costo = [
            (f"Operador Vacío ({fd.get('millas_vacias', 0):.0f} mi × ${r['cxm_vacio']:.4f})",
             r["sueldo_base"]),
            ("Diesel (millas vacías)", r["diesel_usa"]),
        ]
    else:
        filas_costo = [
            (f"Sueldo Base ({fd.get('millas_usa', 0):.0f} mi carg + "
             f"{fd.get('millas_vacias', 0):.0f} mi vac)", r["sueldo_base"]),
...
def _guardar_ruta(r: dict, fd: dict, id_ruta: str, user_id: str, nombre_usuario: str) -> None:
    ...
    fila = {
        ...
        "Modalidad":          fd.get("modalidad", "Desglosada"),
        ...
        "Ingreso_MX_MXP":     fd.get("ingreso_mx", 0.0),
        "Costo_MX_MXP":       fd.get("costo_mx", 0.0),
...
def render() -> None:
    ...
    with st.form("ln_captura_ruta", clear_on_submit=False):
        ...
        with ru1:
            ...
            if es_empty:
                ...
                millas_usa    = st.number_input("Millas Cargadas", min_value=0.0, step=10.0, key="ln_mi_usa")
                millas_vacias = st.number_input("Millas Vacías", min_value=0.0, step=10.0, key="ln_mi_vac")

        with ru2:
            if es_empty:
                # Empty no tiene tarifa al cliente — campos fijos en 0
                moneda_usa   = "USD"
                modalidad    = "Flat"
                cxm_flete    = 0.0
                cxm_fuel     = 0.0
                ...
                if modalidad == "Desglosada":
                    ...
                    tarifa_flat = st.number_input("Tarifa Flat (USD)",
                                                   min_value=0.0, step=50.0, key="ln_tarifa_flat")
                    cxm_flete   = 0.0
                    cxm_fuel    = 0.0
        ...
        if not es_empty and config.get("cruce") in ("opcional", True):
            ...
            aplica_cruce = st.checkbox("¿Aplica cruce?", value=(config["cruce"] is True), key="ln_aplica_cruce")

            if aplica_cruce:
                cx1, cx2 = st.columns(2)
                with cx1:
                    tipo_cruce    = st.selectbox("Tipo de Cruce", ["Propio", "Tercero"], key="ln_tipo_cruce")
                    tipo_carga    = st.selectbox("Carga", ["Cargado", "Vacío"], key="ln_tipo_carga")
                with cx2:
                    moneda_cruce  = st.selectbox("Moneda Cruce", ["USD", "MXP"], key="ln_moneda_cruce")
                    ingreso_cruce = st.number_input("Ingreso Cruce", min_value=0.0, step=5.0, key="ln_ing_cruce")
                    if tipo_cruce == "Tercero":
                        costo_cruce_terc = st.number_input("Costo Cruce Tercero", min_value=0.0,
                                                            step=5.0, key="ln_costo_cruce_terc")
        ...
        if config.get("parte_mx") and not es_empty:
            ...
            with mx1:
                linea_mx   = st.selectbox("Línea MX", ["Propia", "Tercero"], key="ln_linea_mx")
                origen_mx  = st.text_input("Origen MX", key="ln_ori_mx")
                destino_mx = st.text_input("Destino MX", key="ln_dest_mx")
            with mx2:
                moneda_mx  = st.selectbox("Moneda MX", ["MXP", "USD"], key="ln_moneda_mx")
                ingreso_mx = st.number_input("Ingreso Flete MX", min_value=0.0, step=100.0, key="ln_ing_mx")
                if linea_mx == "Tercero":
                    costo_mx = st.number_input("Costo Flete MX", min_value=0.0, step=100.0, key="ln_costo_mx")
        ...
        if not es_empty:
            ...
            st.caption("Captura el monto y marca si Lincoln lo pagó (suma a costo).")
            cols = st.columns(3)
            for i, extra in enumerate(EXTRAS_USA):
                with cols[i % 3]:
                    monto = st.number_input(extra, min_value=0.0, step=10.0,
                                            format="%.2f", key=f"ln_extra_{extra}")
        ...
        # ── Submit ────────────────────────────────────────────────────────
        divider()
        submitted = st.form_submit_button("🔍 Calcular Ruta", type="primary",
    ...
    if submitted:
        ...
        # Convertir a USD si capturaron en MXP
        if not es_empty:
            if modalidad == "Desglosada":
                ...
                ing_x_milla_usd = (tarifa_flat if moneda_usa == "USD" else a_usd(tarifa_flat, tc)) / millas_usa if millas_usa else 0.0
                fuel_sc_usd     = 0.0
        ...
        # Tramo MX siempre en MXP internamente
        if config.get("parte_mx") and not es_empty:
            if moneda_mx == "USD":
    ...
    if r and fd:
        ...
        with col_g:
            if st.button("💾 Guardar Ruta", type="primary", use_container_width=True,
                         key="ln_guardar_ruta"):
